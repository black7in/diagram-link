...
export const generateAndExportXML = (
    ...
    const modelSection = `
          <XMI.content>
                  ...
                  <UML:Namespace.ownedElement>
                      ...
                      <UML:Package name="${diagramName}" xmi.id="EAPK_${idIndex}" isRoot="false" isLeaf="false" isAbstract="false" visibility="public">
                          ...
                          <UML:Namespace.ownedElement>
                              ...
                              ${generateRelationshipsXML(relationshipData, classData)}
              ...
              ${generateDiagramSection(diagramName, classData, relationshipData, associationData)}
        `;
...
const generateClassesXML = (classData) => {
    return classData
        .map(
            (cls) => `
          <UML:Class name="${cls.name}" xmi.id="EAID_${cls.id
...
const generateDiagramElementsXML = (classData) => {
    return classData
        .map(
            (cls, index) => `
          <UML:DiagramElement geometry="Left=${index * 100};Top=${index * 100
                };Right=${index * 100 + 100};Bottom=${index * 100 + 100
        ...
        .map((rel) => {
            ...
            switch (rel.type) {
                ...
                case "-->": // Asociación dirigida
                    relTypeXML = `
              <UML:Association name="${rel.name || ""}" xmi.id="EAID_${rel.id
            ...
            `;
                ...
                case "--|>": // Generalización
                    relTypeXML = `
              <UML:Generalization subtype="EAID_${rel.idFrom}" supertype="EAID_${rel.idTo
                        }" name="${rel.name || ""}" xmi.id="EAID_${rel.id
                        }" visibility="public">
            ...
            `;
                ...
                case "<|--": // Generalización inversa
                    relTypeXML = `
              <UML:Generalization subtype="EAID_${rel.idTo}" supertype="EAID_${rel.idFrom
                        }" name="${rel.name || ""}" xmi.id="EAID_${rel.id
                        }" visibility="public">
                  ${generateModelElementTaggedValue(rel, "Generalization")}
              </UML:Generalization>
            `;
                ...
                case "..|>": // Realización
                    relTypeXML = `
              <UML:Dependency client="EAID_${rel.idFrom}" supplier="EAID_${rel.idTo
                        }" name="${rel.name || ""}" xmi.id="EAID_${rel.id
                        }" visibility="public">
...
const generateConnectionRelationship = (
    ...
    return `
      <UML:${typeRelationship} visibility="public" multiplicity="${relationship.class1Multiplicity
        }" aggregation="${relationship.type === "o--"
            ? "shared"
            : relationship.type === "*--"
                ? "composite"
                : "none"
        }" isOrdered="false" targetScope="instance" changeable="none" isNavigable="true" type="EAID_${relationship.idFrom
        }">
        <UML:ModelElement.taggedValue>
          <UML:TaggedValue tag="containment" value="Unspecified"/>
          <UML:TaggedValue tag="sourcestyle" value="Union=0;Derived=0;AllowDuplicates=0;Owned=0;Navigable=Unspecified;"/>
      ...
      <UML:${typeRelationship} visibility="public" multiplicity="${relationship.class2Multiplicity
        }" aggregation="${relationship.type === ("--o" || "--+")
            ? "shared"
            : relationship.type === "--*"
                ? "composite"
                : "none"
        }" isOrdered="false" targetScope="instance" changeable="none" isNavigable="true" type="EAID_${relationship.idTo
        }">
        <UML:ModelElement.taggedValue>
          <UML:TaggedValue tag="containment" value="Unspecified"/>
          <UML:TaggedValue tag="deststyle" value="Union=0;Derived=0;AllowDuplicates=0;Owned=0;Navigable=Unspecified;"/>
...
const generateModelElementTaggedValue = (relationship, typeRelationship) => {
    ...
    return `
      <UML:ModelElement.taggedValue>
        ...
        <UML:TaggedValue tag="direction" value="${["-->", "o--", "*--", "--+", "--|>", "..|>", "..>"].includes(
        relationship.type
    )
            ? "Source -&gt; Destination"
            : "Unspecified"
        ...
        <UML:TaggedValue tag="src_visibility" value="Public"/>
              <UML:TaggedValue tag="src_multiplicity" value="${relationship.class1Multiplicity
        }"/>
              <UML:TaggedValue tag="dst_multiplicity" value="${relationship.class2Multiplicity
        }"/>
